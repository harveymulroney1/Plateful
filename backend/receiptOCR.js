...
export async function receiptCleaningTest(imagePath){
  ...
  console.log(cleanIngredients(lines));
  return cleanIngredients(lines);
  //console.log(extractProductNames(lines));
};
...
{
  const fuse = new Fuse(recipeToCheck.map(item=>({name:item})),options);
  ...
  let ingredientsCount = recipeToCheck.length;
  console.log("Matches: ",fuzzyMatches);
  console.log("Matches Count: ",matchesCount);
  console.log(((matchesCount/ingredientsCount) * 100)>50 ? "Suggest Recipe":"Don't suggest recipe");
...
const recipeIngr = await scrapeIngrMethod("https://www.bbcgoodfood.com/recipes/marry-me-chicken");
